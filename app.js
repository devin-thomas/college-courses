(function () {
  ...
  function addMaterialDetails() {
    ...
    document.querySelectorAll(".badge, .status-pill").forEach(function (pill) {
      ...
      icon.className = "material-symbol";
      icon.setAttribute("aria-hidden", "true");
      icon.textContent = pill.classList.contains("pending") ? "schedule" : "check_circle";
      pill.prepend(icon);
    });
  }
  ...
  function addCrossLink() {
    ...
    link.href = "https://devin-thomas.github.io/degree-plan/";
    link.setAttribute("aria-label", "Open full degree plan");
    link.innerHTML = '<span>Full Degree Plan</span><i class="material-symbol" aria-hidden="true">arrow_forward</i>';
